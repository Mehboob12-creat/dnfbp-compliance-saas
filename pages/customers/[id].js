  ...
  function downloadRiskPDF() {
    try {
      if (!data?.customer || !data?.latestTransaction || !risk) {
        alert("Risk data not ready yet. Add a transaction first.");
        return;
      }
          ...
          {risk ? (
            <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                <button onClick={saveRisk} style={primaryBtn}>
          ...
          {risk ? (
            <Card title="Risk Assessment (Explainable)">
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 8 }}>
                ...
                {risk.recommendations.edd ? (
                  <Badge label="EDD Required" tone="warn" />
                ) : (
                  <Badge label="EDD Not Required" tone="ok" />
                )}
                {risk.recommendations.str ? (
                  <Badge label="STR Suggested" tone="danger" />
                ) : (
                  <Badge label="STR Not Suggested" tone="ok" />
                )}
                {risk.recommendations.ctr ? (
                  <Badge label="CTR Suggested" tone="danger" />
                ) : (
                  <Badge label="CTR Not Suggested" tone="ok" />
                )}
              </div>
              ...
              <div style={{ display: "grid", gap: 8 }}>
                ...
                <ul style={{ margin: 0, paddingLeft: 18, color: "#0f172a" }}>
                  {risk.recommendations.reasons.map((x, i) => (
                    <li key={i}>{x}</li>
                  ))}
                </ul>
                ...
                >
                  <div style={{ fontWeight: 900, marginBottom: 6 }}>
                    Neutral wording (inspection-safe)
                  </div>
            ...
            <Card title="Risk Assessment">Add a transaction first to calculate risk.</Card>
          )}
        </div>
      </div>
    </div>
...
function calculateRisk(customer, txn) {
  ...
  const filerScore =
    customer.filer_status === "non_filer" ? 20 : customer.filer_status === "unknown" ? 10 : 0;
  ...
  const sourceScore =
    src === "salary"
      ...
      : src === "sale_of_asset"
      ? 10
      : src === "foreign_remittance"
      ? 15
      : src === "inheritance_gift"
      ? 12
      : src === "unknown"
      ? 20
      : 15;
  ...
  const baseScore = filerScore + incomeRatioScore + sourceScore + paymentScore + geoScore + pepScore;
  ...
  const category =
    overallScore <= 30 ? "LOW" : overallScore <= 60 ? "MEDIUM" : overallScore <= 80 ? "HIGH" : "VERY_HIGH";

  const tone = category === "LOW" ? "ok" : category === "MEDIUM" ? "warn" : "danger";
...
function recommendFiling({ txn, overallScore, redFlags, geoTier }) {
  ...
  const ctr = pm === "cash" && amount >= 2000000;

  const edd =
    overallScore > 60 || txn.pep_status === "yes" || geoTier === "high" || redFlags.length >= 2;
